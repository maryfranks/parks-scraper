...
nm.goto(`${baseUrl}/parksguide`)
  ...
  }).then(function(parkFeaturesURLs){
      console.log(parkFeaturesURLs);
      });
  // .then(crawl).catch(bail);
  ...
  if (urls.length < 2) {
    nm.goto(baseUrl + urls.pop())
      ...
      .then(function(result) {
        ...
        console.log(`Finished scraping ${acc.length} dinos to results/ontarioparks.json`);
      })
      .catch(bail);
  } else {
    nm.goto(baseUrl + urls.pop())
...
function scrape(acc) {
  ...
  result['name'] = document.querySelector().innerText;
